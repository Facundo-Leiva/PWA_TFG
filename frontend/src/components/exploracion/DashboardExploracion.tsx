...
type ExplorationView = "list" | "map";

const EXPLORATION_VIEW_STORAGE_KEY = "explorationDashboardView";

function getInitialExplorationView(): ExplorationView {
    return sessionStorage.getItem(EXPLORATION_VIEW_STORAGE_KEY) === "map" ? "map" : "list";
}

export default function DashboardExploracion({ onShowDetail, onExit }: Props) {
    const [view, setView] = useState<ExplorationView>(getInitialExplorationView);
    ...
    useEffect(() => {
        sessionStorage.setItem(EXPLORATION_VIEW_STORAGE_KEY, view);
    }, [view]);
            ...
            <header className="bg-linear-to-r from-blue-600 to-green-600 shadow-lg sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex justify-between items-center gap-3">
                    ...
                    <button
                        type="button"
                        onClick={onExit}
                        className="min-h-10 shrink-0 rounded-lg border border-white/70 px-2.5 sm:px-3 text-sm font-semibold text-white transition-colors hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white"
                        aria-label="Salir del modo exploración"
                        title="Salir del modo exploración"
                    >
